...
async function updateCriteria(config) {
    return fs.promises.writeFile('config/default.json', JSON.stringify(config, null, 2), function writeJSON(error) {
...
async function restartApp() {
    await exec('pm2 start --name container-sweep lib/server.js || pm2 restart all', 
        (error, stdout, stderr) => {
            if (error) 
                console.log(error);
            else
